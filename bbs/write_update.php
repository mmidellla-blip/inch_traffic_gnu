...
if($_POST["bo_table"] != "simple_design" && $_POST["bo_table"] != "simple_design2" && $_POST["bo_table"] != "member" )
{
	if ($wr_content == '') {
...
//담당변호사 리스트 세팅
if($_POST["bo_table"] == "case_box_lawyer" || $_POST["bo_table"] == "case_box_lawyer_img" || $_POST["bo_table"] == "case_box_lawyer_view" || $_POST["bo_table"] == "gallery2" || $_POST["bo_table"] == "case_online"){
	if(isset($_POST["lawyers"]) && is_array($_POST["lawyers"]) && count($_POST["lawyers"]) > 0) {
		$wr_10 = "|";
		for($lawyers_i=0; $lawyers_i < count($_POST["lawyers"]); $lawyers_i++) {
			$wr_10 .= $_POST["lawyers"][$lawyers_i]."|";
		}
	}
}
...
if ($w == '' || $w == 'r') {
   ...
   if ($w == 'r') {
        // 원글 상태 업데이트
        sql_query(" update $write_table set wr_2 = '상담완료' where wr_id = '$wr_id' or wr_id = '$org_wr_id' ");
    } 
    // 새글 INSERT
...
if (!($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
	...
	$mail_wr_tel = $_POST["wr_1"];
	$mail_wr_cate = $_POST["wr_3"];
...
// Naver WCS 스크립트 (online 게시판이 아닌 경우에만)
if ($final_bo_table != 'online') {
?>
<script type="text/javascript" src="//wcs.naver.net/wcslog.js"></script>
<script type="text/javascript">
    if(window.wcs){
    if(!wcs_add) var wcs_add = {};
    wcs_add['wa'] = 's_2e8953b99ff3';
    var _conv = {};
    	_conv.type = 'lead';
    wcs.trans(_conv);
    }
