...
<section class="sub-conts-wrap">
	...
	<section id="bo_w">
		...
		<?php
			//해당글 담당변호사
			$arr_lawyer = explode("|", $wr_10);
			
			$lawyer_sql = " select * from g5_write_lawyer_list ";
			$lawyer_result = sql_query($lawyer_sql);
		?>
		<div class="bo_w_select write_div">
			<label for="lawyers"><b>담당 변호사 선택: </b></label>
			<?php while ($lawyer_row = sql_fetch_array($lawyer_result)) { ?>
				&nbsp;&nbsp;<input type="checkbox" name="lawyers[]" value="<?php echo $lawyer_row["wr_id"]; ?>" <?php echo in_array($lawyer_row["wr_id"], $arr_lawyer)?"checked":"" ?> > <?php echo $lawyer_row["ca_name"]; ?> <b><?php echo $lawyer_row["wr_subject"]; ?></b>
			<?php } ?>
		</div>
