...
<!-- 			<ul class="bo_v_nb">
		...
		<section id="bo_v_atc">
			...
			if($v_img_count) {
				...
				for ($i=0; $i<=count($view['file']); $i++) {
					//if ($view['file'][$i]['view']) {
					if ($view['file'][$i]['view'] && !preg_match("/변호사/",$view['file'][$i]['bf_content'])) {
		...
		<div>
			...
			<?php
				...
				for($lawyer_i=0; $lawyer_i < count($arr_lawyer); $lawyer_i++) {
					...
					$curr_lawyer_cnt++;
					if($curr_lawyer_cnt > 2) break;

					$lawyer_row = sql_fetch("select * from g5_write_lawyer_list where wr_id=".$arr_lawyer[$lawyer_i]." ");
					$lawyer_file = get_file("lawyer_list", $arr_lawyer[$lawyer_i]);

					if(is_array($lawyer_file) && $lawyer_file["count"] > 1) {
						$lawyer_pic = $lawyer_file[1]["path"]."/".$lawyer_file[1]["file"];
					} else {
						$lawyer_pic = "";
					}
			?>
		...
		<div class="view-online">
			...
			<section class="online-form">
				<form action="<?php echo G5_URL ?>/_ok.php" method="post">
					...
					<ul class="input-wrap">
						<li class="tel"><label for="h_name">이름</label><input class="sec07_input" type="text" name="h_name" id="h_name" placeholder="이름을 입력하세요." required/></li>
						<li class="tel"><label for="h_tel">연락처</label><input class="sec07_input" type="text" name="h_tel" id="h_tel" placeholder="연락처를 입력하세요." required/></li>
						<li><label for="wr_5">상담변호사 선택</label>
							<select CLASS="sec07_input" name="wr_5" id="wr_5" required>
								<option value="상담변호사 선택">상담변호사 선택</option>
								<option value="전지민 변호사">전지민 변호사</option>
								<option value="김은진 변호사">김은진 변호사</option>
								<option value="박지현 변호사">박지현 변호사</option>		
								<option value="미정">미정</option>
							</select>
<!--
							<label for="wr_5">상담분야</label>
							
							<select CLASS="sec07_input" name="wr_5" id="wr_5" required>
								<option value="상담분야 선택">상담분야 선택</option>
							</select>
							-->
						</li>
					</ul>
					<p class="tel-info">전화상담 : 평일/주말 09:00 ~ 21:00</p>
					<p class="check"><input type="checkbox" id="chk" name="chk" required/><label for="chk">개인정보 수집 및 이용목적에 동의합니다.</label></p>
					<p class="btn-wrap"><button id='submit_btn'>전화상담 요청</button></p>
				</form>
			</section>
		</div>
	</div>
